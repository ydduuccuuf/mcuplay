...
void LCD_ShowHzString(u16 x, u16 y, u8 *c)
{
	int i, j;
	u8 buffer[32];
	u16 tmp_char=0;
	...
	for (i = 0; i < 16; i++)		 
	{
		...
		for (j = 0; j < 16; j++)
		{
			...
			{
				POINT_COLOR=BLACK;
				LCD_DrawPoint(x + j, y + i);                  //  字符颜色
			}
			else
			{
				POINT_COLOR=WHITE;
				LCD_DrawPoint(x + j, y + i);                 //  背景颜色
			}
		}
		POINT_COLOR=BLACK;
	}
...
void LCD_ShowString_1(u16 x0, u16 y0, u8 *pcStr)
{
    while(*pcStr)
	{
		if(*pcStr > 0xa1)                                               //  显示汉字
		...
		{
			LCD_ShowCharString(x0, y0, pcStr);	
			pcStr +=1;
			x0+= 8;
		}
	}	
